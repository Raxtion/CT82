	...
	case nTagLaserMark:nThreadIndex = 35;
		if (g_IniFile.m_bUseLaserMark)
		{
			if (!m_bLaserLocked)
			{
				...
				if (m_bIsMapCCDLocked == false && g_IniFile.m_nUseTable == 2) g_Motion.AbsMove(Axis_Const::CCD, g_IniFile.m_dTableMarkPosY[!bFront]);
				nThreadIndex++;
			}
		}
...
void __fastcall CMainThread::doSCPicker(int &nThreadIndex)
{
	...
	case 17:
		if (m_listCoverCodeReaderRX.size()>0)
		{
			...
			else if (m_listCoverCodeReaderRX.back() == "2DCODE_DIFFERENCE_ERROR") g_IniFile.m_nErrorCode = 440;
			else
			{
