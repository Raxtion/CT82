        ...
        if(g_pMainThread->m_listTX.size()>0 )
        {
                ...
                for(int nIndex=0;nIndex<4;nIndex++)
                {
                        ...
                        if(strCommand==strSetOffsetLaserCMD[nIndex])
                        {
                                ...
                                //Set Offset
                                bRet=g_Laser.SetToolPosOffset(g_IniFile.m_nLaserProgramNo[nIndex],
                                    0.0 + g_pMainThread->m_tmpProductInfo[nIndex % 2].m_nOffsetX,
                                    0.0 - g_pMainThread->m_tmpProductInfo[nIndex % 2].m_nOffsetY,
                                    g_pMainThread->m_tmpProductInfo[nIndex % 2].m_dOffsetAngle);
                                // bRet=true;
...
{
        if (nIndex == 0 || nIndex == 2) return g_pMainThread->m_tmpProductInfo[false].m_strLotID;
        else return g_pMainThread->m_tmpProductInfo[true].m_strLotID;
...
void __fastcall TfmMain::SpeedButton22Click(TObject *Sender)
{
        ...
        unsigned short nAxisTable[] = { Axis_Const::RTX,Axis_Const::FTX };

        double dPosX = g_IniFile.m_dTableMarkPosX[nTable] - dX*g_IniFile.m_dFMCCDResolution[0];
        double dPosY = g_IniFile.m_dTableMarkPosY[nTable] + dY*g_IniFile.m_dFMCCDResolution[1];

        g_Motion.AbsMove(nAxisTable[nTable], dPosX);
        g_Motion.AbsMove(Axis_Const::CCD, dPosY);

        while (1)
        {
        	if (g_Motion.IsPosDone(nAxisTable[nTable], dPosX) &&
		        g_Motion.IsPosDone(Axis_Const::CCD, dPosY)) break;
        	if (tm1MS.timeUp()) break;
            	Application->ProcessMessages();
        }
...
void __fastcall TfmMain::SpeedButton5Click(TObject *Sender)
{
        ...
        double dPosX = g_IniFile.m_dTableMarkPos2X[nTable] - dX*g_IniFile.m_dFMCCDResolution[0];
        double dPosY = g_IniFile.m_dTableMarkPos2Y[nTable] + dY*g_IniFile.m_dFMCCDResolution[1];
...
void __fastcall TfmMain::ImagePaintBoxMouseDown(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    AddList("繪圖像數座標: X=" + IntToStr(X) + " : Y=" + IntToStr(Y));
}
//---------------------------------------------------------------------------


void __fastcall TfmMain::btnFMCCDFireLaserTestClick(TObject *Sender)
{
    if (!AxisMoveCheck()) return;
    ...
    TSpeedButton *pBtn = (TSpeedButton *)Sender;
    bool bFront = pBtn->Tag;
    int nIndex = g_pMainThread->nThreadIndex[5];
    bool bTableLock; (bFront) ? bTableLock = g_DIO.GetDI(DI::FT_LockerOn) : bTableLock = g_DIO.GetDI(DI::RT_LockerOn);

	pBtn->Enabled = false;
    tm1MS.timeStart(60000);

    g_pMainThread->nThreadIndex[5] = 19;
    g_pMainThread->m_listLog.push_back("系統控制中, 請勿操作...");
    while (true)
    {
        g_pMainThread->doTable(g_pMainThread->nThreadIndex[5], bFront);
        if (g_pMainThread->nThreadIndex[5] == 67) break;
        if (g_IniFile.m_nErrorCode>0 && g_IniFile.m_nErrorCode<1000) break;
        if (tm1MS.timeUp()) break;
        Application->ProcessMessages();
        ::Sleep(10);
    }
    ...
    g_pMainThread->m_bIsMachineStop = true;
    g_pMainThread->nThreadIndex[5] = nIndex;

    (bFront) ? g_DIO.SetDO(DO::FT_Locker, bTableLock) : g_DIO.SetDO(DO::RT_Locker, bTableLock);
