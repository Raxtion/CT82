...
//---------------------------------------------------------------------------
bool __fastcall CTK_Led::SetVoltage(int nCh,int nValue)
{
        char strSend[100]={0};

        int nByte5 = (nValue & 0xff00) >> 8;
        int nByte6 = (nValue & 0xff);
        int nCks=(0x89+0x55+0xaa+0x23+nCh+nByte5+nByte6+0x00) & 0xff;
        sprintf(strSend,"%c%c%c%c%c%c%c%c%c",0x89,0x55,0xaa,0x23,nCh,nByte5,nByte6,0x00,nCks);
        WriteData(strSend,0x89-0x80);
        char strReceive[100]={0};
