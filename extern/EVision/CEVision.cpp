...
__fastcall CEVision::CEVision()
{
        ...
        m_ImageSizeX=2000;//640;
        m_ImageSizeY=1000;//480;
        m_PixelSize=8;
       ...
       // ERectangleGauge1.Attach(&m_frameShape);
        ...
        m_rectAlignment.SetRotatable(true,true);
         ...
         for(int nIndex=0;nIndex<3;nIndex++)
         {
                ...
                m_roiMark[nIndex].SetPlacement(10,10,640,480);

                m_roiMarkPattern[nIndex].Attach(&m_ImageTmpBW8);
                m_roiMarkPattern[nIndex].SetPlacement(10,10,320,240);
         }
         ...
         m_bShowMarkROI[0]=true;
         m_bShowPatternROI[0]=false;
         m_bShowMatchResult[0]=true;

         m_bShowMarkROI[1]=true;
         m_bShowPatternROI[1]=false;
         m_bShowMatchResult[1]=true;

         m_bShowMarkROI[2]=true;
         m_bShowPatternROI[2]=false;
         m_bShowMatchResult[2]=true;


         m_bShowMatchResultAll=true;

         CreateMapArray(TRect(m_roiMark[0].GetOrgX(),m_roiMark[0].GetOrgY(),
                m_roiMark[0].GetOrgX()+m_roiMark[0].GetWidth(),
                m_roiMark[0].GetOrgY()+m_roiMark[0].GetHeight()),
                4,1,10,
                4,4);
}
//---------------------------------------------------------------------------
...
void __fastcall CEVision::LoadTool(char *strFileName,AnsiString strSch,bool bFileType)
{
        ...
        if(pSerializer==NULL /*|| pSerializer->handle_==NULL*/)
        {
...
//---------------------------------------------------------
double __fastcall CEVision::GetMatchPosX(EMatch &theMatch,int nROI,int nIndex)
{
         if(theMatch.GetNumPositions()>0)
         {
                m_pMatchPos=theMatch.GetPosition(nIndex);
                return m_roiMark[nROI].GetOrgX()+m_pMatchPos->m_f32CenterX;
         }
         else return 0;
}
//---------------------------------------------------------
double __fastcall CEVision::GetMatchPosY(EMatch &theMatch,int nROI,int nIndex)
{
         if(theMatch.GetNumPositions()>0)
         {
                m_pMatchPos=theMatch.GetPosition(nIndex);
                return m_roiMark[nROI].GetOrgY()+m_pMatchPos->m_f32CenterY;
         }
         else return 0;
}
//---------------------------------------------------------
